...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/12 12:34:35 by pzau              #+#    #+#             */
/*   Updated: 2024/08/12 17:18:19 by pzau             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_map_line_one(t_vars *vars, char **map, int y, int largura)
{
	...
	{
		cell = map[y][x];
		img = get_image_for_cell_one(vars, cell);
		if (img)
			mlx_put_image_to_window(vars->mlx,
				vars->win_level_one, img, x * PIX, y * PIX);
		x++;
	}
}
...
void	render_map_one(t_vars *vars, char **map, t_dimensoes dim)
{
	...
	while (y < dim.altura)
	{
		render_map_line_one(vars, map, y, dim.largura);
		y++;
	}
}
